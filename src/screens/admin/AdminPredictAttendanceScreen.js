...
export default function AdminPredictAttendanceScreen({ route, navigation }) {
  ...
  const capacity = factors.capacity || 0;
  const prediction = predictionData.prediction || 0;
  const percentFull = capacity > 0 ? ((prediction / capacity) * 100).toFixed(1) : 0;
  const weatherIcon = factors.rainfall > 0 ? "🌦️" : (factors.temperature > 25 ? "☀️" : "☁️");
  const isWeekend = factors.day_of_week === 0 || factors.day_of_week === 6;
  const monthName = new Date(2000, (factors.month || 1) - 1).toLocaleString('default', { month: 'short' });
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View style={[styles.factorsSection, { opacity: fadeAnim }]}>
          ...
          <View style={styles.factorGrid}>
            ...
            <View style={styles.factorCard}>
              ...
              <Text style={styles.factorValue}>T{factors.home_tier || '?'} v T{factors.away_tier || '?'}</Text>
              <Text style={styles.factorName}>Franchise Tiers</Text>
            </View>

            <View style={styles.factorCard}>
              <Text style={styles.factorIcon}>{weatherIcon}</Text>
              <Text style={styles.factorValue}>{factors.temperature}°C</Text>
              <Text style={styles.factorName}>Expected Temp</Text>
            </View>

            <View style={styles.factorCard}>
              <Text style={styles.factorIcon}>{isWeekend ? '📅' : '📆'}</Text>
              <Text style={styles.factorValue}>{isWeekend ? 'Yes' : 'No'}</Text>
            ...
            <View style={styles.factorCard}>
              <Text style={styles.factorIcon}>🌧️</Text>
              <Text style={styles.factorValue}>{factors.rainfall} mm</Text>
              <Text style={styles.factorName}>Rainfall</Text>
            </View>

            <View style={styles.factorCard}>
              <Text style={styles.factorIcon}>🗓️</Text>
              <Text style={styles.factorValue}>{monthName}</Text>
              <Text style={styles.factorName}>Seasonality</Text>
            </View>
        ...
        <Animated.View style={[styles.insightCard, { opacity: fadeAnim }]}>
            ...
            <View style={styles.insightTextWrap}>
              <Text style={styles.insightTitle}>Universal AI Insight</Text>
              <Text style={styles.insightDesc}>
                This prediction is generated by our Universal Random Forest Regressor trained strictly on authentic historic data. It evaluates Franchise Tiers, Venue Capacity, and Live Open-Meteo Weather to predict the exact stadium fill rate.
              </Text>
            </View>
        </Animated.View>
